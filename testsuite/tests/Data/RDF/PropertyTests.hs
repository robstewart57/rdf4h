...
instance
  ...
  arbitrary = do
    ...
    return SingletonGraph {rdfGraph = (mkRdf [t] baseU pref (Just 0))}
...
p_mkRdf_triplesOf ::
  ...
  Maybe Int ->
  Bool
p_mkRdf_triplesOf _mkRdf ts bUrl pms nextId =
  uordered (triplesOf (_mkRdf ts bUrl pms nextId)) == uordered ts
...
p_mkRdf_no_dupes ::
  ...
  Maybe Int ->
  Bool
p_mkRdf_no_dupes _mkRdf ts bUrl pms nextId = null ts || (sort result == uordered ts)
  where
    tsWithDupe = head ts : ts
    result = uniqTriplesOf $ _mkRdf tsWithDupe bUrl pms nextId
...
p_query_match_none ::
  ...
  Maybe Int ->
  Bool
p_query_match_none _mkRdf ts bUrl pms nextId = uordered ts == uordered result
  where
    result = query (_mkRdf ts bUrl pms nextId) Nothing Nothing Nothing
...
p_add_triple ::
  ...
  Triple ->
  Bool
p_add_triple _mkRdf ts bUrl pms nextId newTriple =
  uordered (newTriple : triplesOf oldGr) == uordered (triplesOf newGr)
  where
    oldGr = _mkRdf ts bUrl pms nextId
...
p_remove_triple ::
  ...
  Triple ->
  Bool
p_remove_triple _mkRdf ts bUrl pms nextId tripleToBeRemoved =
...
p_remove_triple_from_graph ::
  ...
  Maybe Int ->
  Property
p_remove_triple_from_graph _mkRdf ts bUrl pms nextId =
  classify (null ts) "p_remove_triple_from_graph tests were trivial" $
    forAll (tripleFromGen triplesOf (_mkRdf ts bUrl pms nextId)) f
  where
    f :: Maybe Triple -> Bool
    f Nothing = True
...
instance (Rdf rdf) => Arbitrary (RDF rdf) where
  arbitrary = do
    ...
    baseU <- oneof [return (Just baseU'), return Nothing]
    ts <- arbitraryTs
    return $ mkRdf ts baseU prefix (Just 0)
...
p_reverseRdfTest _mkRdf =
  ...
  where
    rdf = _mkRdf ts (Just $ BaseUrl "file://") (ns_mappings mempty) (Just 0)
    ts :: [Triple]
    ts =
      [ Triple
