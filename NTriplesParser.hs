...
nt_ntripleDoc :: GenParser Char st ([Maybe (IO Triple)], Maybe BaseUrl, PrefixMappings)
nt_ntripleDoc = 
  many nt_line >>= \lines ->
...
nt_line :: GenParser Char st (Maybe (IO Triple))
...
nt_comment :: GenParser Char st (Maybe (IO Triple))
...
-- whitespace.
nt_triple :: GenParser Char st (Maybe (IO Triple))
nt_triple    = 
  do
    subj <- nt_subject
    many1 nt_space
...
nt_predicate :: GenParser Char st (IO Node)
...
nt_literal    = 
  do 
    ...
    rt <- do {char '@'; lng <- nt_language; return (Just (Left lng))} <|> 
          ...
          do {return Nothing}
    case rt of
      Nothing              -> return (return $! PlainL str)
      (Just (Left lng))    -> return (return $! PlainLL str lng)
