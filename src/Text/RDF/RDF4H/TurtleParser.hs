...
type ParseState =
  (Maybe BaseUrl,    -- the current BaseUrl, may be Nothing initially, but not after it is once set
   ...
   Maybe Predicate,  -- current predicate node, if we've parsed a predicate but not finished the triple
...
_modifyState mb_bUrl mb_n mb_pms mb_subj mb_pred mb_trps =
  do (_bUrl, _dUrl, _n, _pms, _s, _p, _ts, genMap) <- get
...
initialState bUrl docUrl = (bUrl, docUrl, 1, PrefixMappings Map.empty, Nothing, Nothing, Seq.empty,Map.empty)
