...
{- NOTE: this module will be removed in future. It is based on arrows. The
 RDF/XML parser in this library is in Text.RDF.RDF4H.XmlParser and is based on
 xmlbf>=0.7. This arrows based module XmlParserHXT is not exported or used
 internally.
-}
...
import Data.Maybe
import Data.RDF.IRI
import Data.RDF.Types (BaseUrl (..), Node (BNodeGen), Object, ParseFailure (ParseFailure), Predicate, PrefixMappings (..), RDF, Rdf, RdfParser (..), Subject, Triple (..), Triples, bnode, lnode, mkRdf, plainL, plainLL, typedL, unode, unodeValidate)
import Data.Text (Text)
import qualified Data.Text as T -- (Text,pack,unpack)
import qualified Data.Text.IO as TIO
import Data.Typeable
import Text.RDF.RDF4H.ParserUtils hiding (rdfType)
import Text.XML.HXT.Core (ArrowIf, ArrowXml, IfThen ((:->)), XmlTree, arr2A, choiceA, constA, expandURI, first, getAttrValue, getAttrValue0, getAttrl, getChildren, getName, getText, hasAttr, hasAttrValue, hasName, ifA, isA, isElem, isText, isWhiteSpace, listA, mkelem, neg, orElse, runSLA, sattr, second, this, unlistA, xreadDoc, xshow, (<+>), (>.), (>>.))
...
parseFile' bUrl dUrl fpath =
  TIO.readFile fpath Data.Functor.<&> parseXmlRDF bUrl dUrl
...
parseXmlRDF bUrl dUrl xmlStr = case runParseArrow of
  ...
  where
    runParseArrow = runSLA (xreadDoc >>> isElem >>> addMetaData bUrl dUrl >>> getRDF) initState (T.unpack xmlStr)
...
addMetaData bUrlM dUrlM =
  mkelem
    ...
    [arr id]
  where
    mkSource (Just dUrl) = [sattr "source" (T.unpack dUrl)]
    mkSource Nothing = []
...
getRDF = proc xml -> do
  ...
  where
    toAttrMap = (getAttrl >>> (getName &&& (getChildren >>> getText))) >. id
...
parseDescription =
  ...
  where
    readTypeTriple :: (ArrowXml a) => LParseState -> a XmlTree Triple
    readTypeTriple state = getName >>> arr (Triple (stateSubject state) rdfType . unode . T.pack)
    replaceLiElems acc n (Triple s p o : rest)
      | p == (unode . T.pack) "rdf:li" =
...
parsePredicatesFromChildren ::
  forall a.
  (ArrowXml a, ArrowState GParseState a) =>
...
parsePredicatesFromChildren =
  ...
  where
    ...
    validPropElemNames state = proc (predXml) -> do
...
parseObjectsFromChildren ::
  forall a.
  (ArrowIf a, ArrowXml a, ArrowState GParseState a) =>
  LParseState ->
  Predicate ->
  a XmlTree Triple
...
updateState =
  ifA (second (hasAttr "xml:lang")) (arr2A readLang) (arr id)
    >>> ifA (second (hasAttr "xml:base")) (arr2A readBase) (arr id)
  where
    readLang state = (getAttrValue0 "xml:lang" >>> arr (\lang -> state {stateLang = Just lang})) &&& arr id
...
parseAsLiteralTriple state =
  (nameToUNode &&& (xshow getChildren >>> arr (mkTypedLiteralNode rdfXmlLiteral)))
    >>> arr (attachSubject (stateSubject state))

mkCollectionTriples :: forall a. (ArrowXml a) => a [(Triple, Node)] Triples
mkCollectionTriples = arr (mkCollectionTriples' [])
  where
    mkCollectionTriples' [] ((Triple s1 p1 o1, n1) : rest) =
...
getTypedTriple state =
  nameToUNode &&& (attrExpandURI state "rdf:datatype" &&& xshow getChildren >>> arr (\(t, v) -> mkTypedLiteralNode (T.pack t) v))
    >>> arr (attachSubject (stateSubject state))
...
getNodeIdTriple ::
  ...
  LParseState ->
  a XmlTree Triple
getNodeIdTriple state =
  nameToUNode &&& (getAttrValue "rdf:nodeID" >>> (arrL (maybeToList . xmlName)) >>> arr (bnode . T.pack))
    >>> arr (attachSubject (stateSubject state))
...
mkNode state =
  choiceA
    [ hasAttr "rdf:about" :-> (attrExpandURI state "rdf:about" >>> mkUNode),
...
attrExpandURI state attr = getAttrValue attr &&& baseUrl >>> my_expandURI
  where
    baseUrl = constA (case stateBaseUrl state of BaseUrl b -> T.unpack b)
...
mkUNode =
  choiceA
    [ (arr (isJust . unodeValidate . T.pack)) :-> (arr (unode . T.pack)),
...
mkRelativeNode s =
  (getAttrValue "rdf:ID" >>> (arrL (maybeToList . xmlName)) >>> arr ('#' :)) &&& baseUrl
    >>> expandURI
    >>> arr (unode . T.pack)
  where
    baseUrl = constA (case stateBaseUrl s of BaseUrl b -> T.unpack b)
...
mkBlankNode =
  nextState (\gState -> gState {stateGenId = stateGenId gState + 1})
