...
data instance RDF AdjHashMap = AdjHashMap (TMaps, Maybe BaseUrl, PrefixMappings, Int)
...
showGraph' ((AdjHashMap ((spoMap, _), _, _, _))) =
  let ts = concatMap (uncurry tripsSubj) subjPredMaps
        where
...
baseUrl' (AdjHashMap (_, baseURL, _, _)) = baseURL

prefixMappings' :: RDF AdjHashMap -> PrefixMappings
prefixMappings' (AdjHashMap (_, _, pms, _)) = pms

addPrefixMappings' :: RDF AdjHashMap -> PrefixMappings -> Bool -> RDF AdjHashMap
addPrefixMappings' (AdjHashMap (ts, baseURL, pms, nextId)) pms' replace =
  let merge = if replace then flip (<>) else (<>)
   in AdjHashMap (ts, baseURL, merge pms pms', nextId)

empty' :: RDF AdjHashMap
empty' = AdjHashMap ((mempty, mempty), Nothing, PrefixMappings mempty, 0)

mkRdf' :: Triples -> Maybe BaseUrl -> PrefixMappings -> Maybe Int -> RDF AdjHashMap
mkRdf' ts baseURL pms i = AdjHashMap (mergeTs (mempty, mempty) ts, baseURL, pms, fromMaybe (bnodeGenCount ts) i)

addTriple' :: RDF AdjHashMap -> Triple -> RDF AdjHashMap
addTriple' (AdjHashMap (tmaps, baseURL, pms, nextId)) t =
  let newTMaps = mergeTs tmaps [t]
   in AdjHashMap (newTMaps, baseURL, pms, nextId)

removeTriple' :: RDF AdjHashMap -> Triple -> RDF AdjHashMap
removeTriple' (AdjHashMap ((spo, ops), baseURL, pms, nextId)) (Triple s p o) =
  AdjHashMap (new_tmaps, baseURL, pms, nextId)
  where
...
bnodeGen' :: RDF AdjHashMap -> (Node, RDF AdjHashMap)
bnodeGen' (AdjHashMap (tmaps, base, prefixes, next)) =
  (BNodeGen next, AdjHashMap (tmaps, base, prefixes, (next + 1)))

-- 3 following functions support triplesOf
triplesOf' :: RDF AdjHashMap -> Triples
triplesOf' (AdjHashMap ((spoMap, _), _, _, _)) = concatMap (uncurry tripsSubj) subjPredMaps
  where
    subjPredMaps = HashMap.toList spoMap
...
select' (AdjHashMap ((_, ops), _, _, _)) Nothing p o = selectSPO o p Nothing (\a b c -> Triple c b a) ops
select' (AdjHashMap ((spo, _), _, _, _)) s p o = selectSPO s p o Triple spo
...
query' (AdjHashMap ((_, ops), _, _, _)) Nothing p o = querySPO o p Nothing (\a b c -> Triple c b a) ops
query' (AdjHashMap ((spo, _), _, _, _)) s p o = querySPO s p o Triple spo
